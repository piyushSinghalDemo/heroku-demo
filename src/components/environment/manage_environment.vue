    ...
    <v-container grid-list-md text-xs-center>
      <v-form v-model="valid" ref="form">
        <ul class="breadcrumb">
          ...
          <li><span style="font-weight:400;font-size:15px;"><input type="text" :rules="namedRules" v-model="name" @blur="untitle_environment" /></span></li>
        </ul>
        <v-flex style="margin-top:1%">     
          <v-card>
            <v-layout row wrap justify-center>
              <v-flex xs10>
                <v-layout row wrap>
                  <v-flex text-sm-left xs4 style="margin-top: 2.5%">
                    <label class="v-label">Select Environment Type</label>
                  </v-flex>
                  <v-flex xs8 >
                     <v-radio-group v-model="environment_type"  required row>
                        <v-radio v-for="n in environment_type_list" :key="n.id" :label="n.name"  :value="n.id"></v-radio>
                      </v-radio-group>
                  </v-flex>
                  <v-flex text-sm-left xs4 style="margin-top: 2.5%">
                    <label class="v-label" >Select Database Type</label>
                  </v-flex>
                  <v-flex text-sm-left xs8>
                     <v-radio-group v-model="database_type" required row @change="get_data_source_by_db(database_type)">
                        <v-radio v-for="n in database_type_list" :key="n.id" :label="n.name"  :value="n.id"></v-radio>
                      </v-radio-group>
...
=======
                  <v-flex  text-sm-left xs6>
                     <v-autocomplete clearable autocomplete :items="target_data_source_list" item-text="datasource_name" item-value="datasource_id"  v-model="target_data_source" :rules= "requiredTargetDatasource" label="Select Target Datasource" style="margin-left:10px;"></v-autocomplete>
                  </v-flex>
                  <v-flex text-sm-left xs6>
                       <v-autocomplete clearable autocomplete :items="engine_data_source_list" item-text="datasource_name" item-value="datasource_id" v-model="engine_data_source" :rules="requiredEngineDatasource" label="Select Engine Datasource" style="margin-right:10px;"></v-autocomplete>
                  </v-flex>
                  <v-flex text-sm-left xs6>
                     <v-autocomplete clearable autocomplete :items="catalog_datasource_list" item-text="datasource_name" item-value="datasource_id"  :rules="catalogRules" v-model="catalog_data_source" label="Select Catalog Datasource" style="margin-left:10px;"></v-autocomplete>
                  </v-flex>
                  <v-flex text-sm-left xs4 style="margin-top: 2.5%">
                    <label class="v-label">Select Database Location</label>
                  </v-flex>
                  <v-flex text-sm-left xs8>
                   <v-radio-group v-model="dblocation" required row>
                      <v-radio v-for="n in database_location_list" :key="n.id" :label="n.name"  :value="n.id"></v-radio>
                    </v-radio-group>
                  </v-flex>
                  <v-flex text-sm-left xs6>
                    <v-textarea name="input-7-1" label="Description" v-model="description" rows=1></v-textarea>
>>>>>>> 143e7fc0d6e2b98d2e8ce40376ff4db57ac6e09c
                  </v-flex>
                </v-layout>
              </v-flex>
...
=======
import {GETENVIRONMENTDETAILS, GETDATASOURCEBYBDTYPE, CATALOG_DATA_SOURCE_BY_CLIENT_ID, ADDENVIRONMENT, UPDATE_ENVIRONMENT} from '../../data/url_constants.js'
...
=======
    ...
    get_environment_details(){
      var environment_end_point = config.ENVIRONMENT_API_URL + GETENVIRONMENTDETAILS + this.$route.params.environment_id
      getToServer(this, environment_end_point).then(envirnentResponse => {
        this.environment_type_list = envirnentResponse.environment_dict.database
        this.database_type_list = envirnentResponse.database_name_dict.database
        this.database_location_list = envirnentResponse.db_location_dict.db_location
        if(this.$route.params.environment_id){
            var data = envirnentResponse
            this.environment_type = data.environment_details.type
            this.name = data.environment_details.name
            this.database_type = data.environment_details.environment_database_type
            this.get_data_source_by_db(data.environment_details.environment_database_type)
            this.source_data_source = data.environment_details.source_data
            this.target_data_source = data.environment_details.target_data
            this.engine_data_source =data.environment_details.engine_database
            this.catalog_data_source = data.environment_details.catalog_data
      ...
      postToServer(this, data_source_endpoint, user_input, true).then(datasourceResponse => {
         ...
         if(response_data){
          ...
          if(error_response){
            ...
            // this.snackbartext = SERVER_ERROR;
=======
      postToServer(this, data_source_endpoint, user_input).then(datasourceResponse => {
         this.source_data_source_list = this.target_data_source_list = this.engine_data_source_list =  datasourceResponse.datasource
        }).catch(error_response => {
          if(error_response){
            this.snackbar = true
            this.colorValue = 'error'
