        ...
        <v-layout row wrap>
          <v-flex xs12 pl-3>
            <ul class="breadcrumb">
              ...
              <li>
                <select v-model="Environment" v-on:change="getAllArchieves"
...
import {GET_ALL_ENV, GETALLARCHIVES, CREATE_NEW_JOB,GETBUSINESS_OBJECT_BY_ENV,VALIDATE_ACTIVITY_FILTER} from './../../../data/url_constants.js';
...
export default {
  ...
  data: function () {
    return {
      ...
      tableList: {
        headers: [
        { text: 'Archive ID', value:'step_detail_id', width: '130px', sortable: false,align:'right'},
        { text: 'Business Object', value: 'business_object_name', width: '260px',cssClass:'w16'},
        { text: 'Ver', value: 'business_object_revision', width: '80px',align:'right'},
        { text: 'Data Older Than' , value: "data_older_than", width: '170px', cssClass:'w16'}, 
        { text: 'Age', value: "age", width: '100px'}, 
        { text: 'Can Dispose On' , value: "can_dispose_on", width: '170px', cssClass:'w16'}, 
        { text: 'Driver Count', value: 'driver_table_count', width: '150px', align:'right', cssClass:'w16' },
        { text: 'Created Date', value: 'created_date', width: '150px',  cssClass:'w16' },
        { text: 'Created by Job', value:'created_by_job',  width: '170px',align:'right', cssClass:'w16'},
        ],
        actions:[],
  ...
  mounted() {
   ...
   {'text':'restore','key':"activity_type",'cmpValue':"Archival",role:this.userRole.is_superadmin || getUserRole(this.userRole,JOB_MANAGEMENT_SERVICE,EXECUTE_ACTION)},
   {'text':'dispose','key':"activity_type",'cmpValue':"Archival",role:this.userRole.is_superadmin || getUserRole(this.userRole,JOB_MANAGEMENT_SERVICE,EXECUTE_ACTION)},
   ...
   if( env_value && env_value.id != '0' && env_value.id !='all_env')
      ...
      this.getAllArchieves(this.envIpJson)
    ...
    methods: {
      ...
      filterData(filterArray){
        ...
        this.getAllArchieves(inputJson);
      },
      ...
      getAllArchieves () {
        ...
        if(!this.Environment.id)
          ...
          // var bo_id = this.BusinessObject.business_object_id;
        ...
        postToServer(this, config.ENGINE_API_URL + GETALLARCHIVES, data).then(response => {
          var data = response
          if(data){
            _this.archivies_list = data.result;
            var total_count = data.total
            _this.tableList.rows = _this.archivies_list
            this.tableList.total_count = total_count;
              // _this.tableList.rows.map(function(obj){
              //   obj.can_dispose = 'N'
              //   if(obj.can_dispose_on)
              //     var current_date= moment().format('YYYY/MM/DD')
              //     if(current_date > obj.can_dispose_on)
              //       obj.can_dispose = 'Y'
              // })
            }
            _this.hideLoader();
        ...
        CreateNewJob(){
         ...
         ShowData(){
          ...
          if(filter_data.length === 1 && (!filter_data[0].column_name ||!filter_data[0].operator))
            filter_data = []
          this.$router.push({ name: 'showacitvitydata', params: {'activity_ids':_.map(this.selectedRows,'activity_id'),'is_action':false,'table_name':record.catalog_table_name,'env_id':this.Environment.id,'env_name':this.Environment.name,'bo_name':record.business_object_name,'activity':'Archive','filter_data':filter_data,'unique_identifiers':record.unique_identifiers,'from_archive':true}});
        },
        PerformOps(type){
