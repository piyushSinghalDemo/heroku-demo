		...
		<v-flex xs12>
		  	<v-card>
				...
				<div style="float:left;margin-top:7px;">
					<span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;"> Home /</span> &nbsp;<span style="font-weight:400;font-size:15px;">Workflow Template List</span>
					</div>
				...
				<template slot="items" slot-scope="props" id="taskList">
				  	...
				  	<td class="text-xs-left">{{ props.item.approver_details }}</td>
				  	<td class="text-xs-left">
						<v-tooltip bottom >
              <v-icon slot="activator" class="iconFormat" >fa-copy</v-icon>
            <span>Copy</span>
            </v-tooltip>
						<router-link :to="{ name: 'editWorkflowTemplate', params: { workflow_id: props.item.id}}">
							<v-tooltip bottom>
		...
		data () {
		  return {
			...
			headers: [
			  ...
			  { text: 'Approver Details', value: 'approver_details' },
