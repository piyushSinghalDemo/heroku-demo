    ...
    <div id="diagram">
		...
		<!-- <v-flex xs10 ml-3> -->
        	<div style="text-align: left;">
                <span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;"> Home /</span> &nbsp;<span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;">Process Designer /</span>&nbsp;<span style="font-weight:400;font-size:15px;"><input type="text" v-model='definitionname'/></span><span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;margin-left:5px;">{{savedstatus}}</span>
		...
		<v-layout raw wrap>
			<!-- <v-flex xs3> -->
				<div role="tablist" style="width:25%;" id="tablist">
	                    <b-card no-body class="mb-1">
	                    <b-card-header header-tag="header" class="p-1" role="tab">
	                        <b-btn block href="#" v-b-toggle.accordion1><font color='white'>Basic Steps </font> </b-btn>
	                    </b-card-header>
	                    ...
	                    <b-collapse id="accordion2" visible accordion="my-accordion" role="tabpanel">
	                     ...
	                     <b-card no-body class="mb-1">
	                    <b-card-header header-tag="header" class="p-1" role="tab">
	                        <b-btn block href="#" v-b-toggle.accordion3 > <font color='white'>Bussiness Objects</font></b-btn>
	                    </b-card-header>
	                    <b-collapse id="accordion3" visible accordion="my-accordion" role="tabpanel">
	                        <b-card-body>
	                            <div id="bussinesspaper"></div>
	                        </b-card-body>
	                    </b-collapse>
	                    </b-card>
	                  </div>
			<!-- </v-flex> -->
			<!-- <v-flex xs9> -->
				<div style="width:75%;" id="main_container" ref="main_container">
					<div id="paper" style="left:10px;transform-origin: top left;transform: scale(1);" @contextmenu.prevent="$refs.ctxMenu.open" ref="elpaper"></div>

					<v-icon id="close_tablelist_icon" @click.stop="perform_collapse('close')" title="Close Control Panel" style="font-size:40px;position:absolute;top:16%;left:26%;" class="glyphicon" ref="closetablelisticon">chevron_left</v-icon> 


					<v-icon id="open_tablelist_icon" @click.stop="perform_collapse('open')" title="Open Control Panel" style="display:none;position: absolute; top: 16%;left:1%; font-size:40px;" ref="opentablelisticon">chevron_right</v-icon> 
		...
		<context-menu id="context-menu" ref="ctxMenu">
			<li style="font-weight:bold" @mouseover="MouseOverEvent($event,'selection')" @mouseout="MouseOutEvent($event)" v-bind:class="{disabled : !is_selected}" @click="ContextMenuClick('cut')" ref="elcut">Cut</li>
              ...
              <v-dialog v-model="dialog" fullscreen transition="dialog-bottom-transition" :overlay="false" scrollable >
              	<dataselection :parentdigramobj='parentdigramobj' @update-object="perform_popup_close"></dataselection>
              </v-dialog>
            </v-flex>
...
=======
	...
	import {getStepList,getJDCellsList,getDefinedBusinessObjectList} from "../../methods/flowcharts/shapes.js"
	...
	import {ManageScroll,MoveLens,MoveLensOnScroll,UnselectCell,isLeftMouseButtonDown,handleKeyevents,getStencilPaper,drapdroponmainpaper} from "../../methods/flowcharts/papers.js" 
...
=======
    ...
    export default {
	  	components: { contextMenu, Breadcrumb,dataselection },
		data() {
			return{
				// graph:"",
				parentdigramobj:'',
...
=======
		...
		// Canvas from which take shapes
		  ...
		  var BussinessGraph = new joint.dia.Graph,
		  businessPaper = getStencilPaper('#bussinesspaper',BussinessGraph);

		stencilPaper.on('cell:pointerdown', function(cellView, e, x, y) {
			  performZoomActions(_this,'reset')
			  performPanningActions([_this.paperpanAndZoom,_this.minimappanAndZoom],'reset')
			  drapdroponmainpaper(cellView, e, x, y,graph,paper)
		});
		...
		stencilGraph.addCells(getStepList(_this))
		JDGraph.addCell(getJDCellsList(_this))
		BussinessGraph.addCell(getDefinedBusinessObjectList(_this))
		_this.timer =setInterval(_this.AutoSaveGraph,3000)
		},
	methods: {
>>>>>>> 143e7fc0d6e2b98d2e8ce40376ff4db57ac6e09c
...
=======
		...
		perform_popup_close(data_from_selection){
			this.doubled_clicked_el.attr('text/text', data_from_selection.stepname)
			this.dialog = false;
		},
		perform_collapse(action){
		  debugger;
		  ...
		  var lens_width = lens.style.width
          if(action === 'close'){
          	this.$refs.closetablelisticon.style.display="none"
          	this.$refs.opentablelisticon.style.display=""
            ...
            main_container.style.width="75%";
			this.$refs.closetablelisticon.style.display=""
          	this.$refs.opentablelisticon.style.display="none"
          	lens_width =main_container.clientWidth * this.small_paper_scale 
          }
          lens.style.width = Math.round(lens_width)+"px"
      	},
