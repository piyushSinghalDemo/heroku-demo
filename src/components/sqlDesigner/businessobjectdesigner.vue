...
=======
    <div id="diagram">
    	<div style="text-align: left;">
            <span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;"> Home /</span> &nbsp;<span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;">Business Object /</span>&nbsp;<span style="font-weight:400;font-size:15px;"><input type="text" v-model='businessobjectname'/></span>
    	</div>
        <!-- </v-flex> -->
		<v-layout raw wrap>
			<v-flex xs12>
				<div id="main_container" ref="main_container">
					<div id="paper" @contextmenu.prevent="$refs.ctxMenu.open" ref="elpaper" style="transform-origin: top left;transform: scale(1);"></div>

					<div style="position: absolute; top: 115px; font-size:30px;">
					    <button @click="AddNewCell()" title="Add New">
					...
					<div style="position: absolute; top: 490px; right: 3%;font-size:30px;">
					    ...
					    <button @click="ZoomAction('zoom_reset')" title="reset zoom">
					     ...
					     <button @click="ZoomAction('zoom_out')" title="zoom out">
					     <v-icon style="font-size:30px;">zoom_out</v-icon> </button>
					 </div>
				</div>
			</v-flex>
		</v-layout>
		<context-menu id="context-menu" ref="ctxMenu">
			<li style="font-weight:bold" @mouseover="MouseOverEvent($event)" @mouseout="MouseOutEvent($event)"@click="ContextMenuClick('new')" ref="elnew">Add New</li>
			<li style="font-weight:bold" @mouseover="MouseOverEvent($event,'selection')" @mouseout="MouseOutEvent($event)" v-bind:class="{disabled : !is_selected}" @click="ContextMenuClick('cut')" ref="elcut">Cut</li>
  			<li style="font-weight:bold" @mouseover="MouseOverEvent($event,'selection')" @mouseout="MouseOutEvent($event)" v-bind:class="{disabled : !is_selected}" @click="ContextMenuClick('copy')" ref="elcopy">Copy</li>
  			<li style="font-weight:bold" @mouseover="MouseOverEvent($event,'copied')" @mouseout="MouseOutEvent($event)" @click="ContextMenuClick('paste')" v-bind:class="{disabled : !is_cut_or_copied}" ref="elpaste">Paste</li>
  			<li style="font-weight:bold" @mouseover="MouseOverEvent($event,'selection')" @mouseout="MouseOutEvent($event)" v-bind:class="{disabled : !is_selected}" @click="ContextMenuClick('delete')" ref="eldelete">Delete</li>
  			<li style="font-weight:bold" @mouseover="MouseOverEvent($event,'selection')" @mouseout="MouseOutEvent($event)" @click="ContextMenuClick('rename')" v-bind:class="{disabled : !is_selected}" ref="elrename">Rename</li>
		</context-menu>
		  	<v-flex xs12>      
              <v-dialog v-model="dialog" width="400" transition>
                <v-flex>
                  <v-card class="rounded-card">
                    <v-toolbar dark dense absolute style="height:17%">
                     <v-flex class="text-md-center">Step Details</v-flex>                        
                    </v-toolbar>
                    ...
                    <v-flex>
                      <v-form class="text-center">
                        ...
                        <v-flex  style="margin-top:12%">
                          <v-text-field v-model='step_name' label="Enter step name"></v-text-field>
                        </v-flex>
                        <v-flex>
                          <v-text-field v-model='step_description' label="Enter Description"></v-text-field>
                        </v-flex>
                        <v-btn  @click="ChangeStepName()" color="success" large block>
                          Submit</v-btn>
                        </v-container>
                      </v-form>
                    </v-flex>  
              ...
              <v-dialog v-model="link_dialog" width="400" transition>
                <v-flex>
                  <v-card class="rounded-card">
                    <v-toolbar dark dense absolute style="height:20%">
                     <v-flex class="text-md-center">Relationship</v-flex>                        
                    </v-toolbar>
                    <v-flex class="text-md-center"></v-flex>
                    <v-flex>
                      <v-form class="text-center">
                        <v-container fluid grid-list-md>
		 				<v-flex style="margin-top:12%">
		                    <v-radio-group v-model="link_relationship" row hide-details>
		                       <v-radio v-for="n in link_relationshiplist" :key="n.id" :label="n.name"  :value="n.id" class="label-margin"></v-radio>
		                     </v-radio-group>
		                 </v-flex>
                        <v-btn  @click="ChangeRelationships()" color="success" large block>
                          Submit</v-btn>
                        </v-container>
                      </v-form>
                    </v-flex>  
...
<script>
	...
	import {getDefaultLinkProperies,RestrictCyclicConnection,ValidateSourceTarget,ValidateOutgoingLink,ValidateIncomingLink} from "../../methods/flowcharts/links.js"

	import {PerformCellSelection,PrepareContextMenu,KeepCellInsidePaper} from "../../methods/flowcharts/cells.js" 

	import {ManageScroll,MoveLens,MoveLensOnScroll,UnselectCell,isLeftMouseButtonDown,handleKeyevents} from "../../methods/flowcharts/papers.js" 
    ...
    export default {
	  	components: { contextMenu, Breadcrumb },
		data() {
			return{
				// graph:"",
				main_paper:'',
  		...
  		mounted() {        	
			...
			var selected;
			 paper.on("link:connect", function(linkView) {
			 	reset_port_markup(_this.selected_el.model,_this.port_reset_value)
				UnselectCell(_this,selected)
			 	var is_cycle_exists =  RestrictCyclicConnection(graph,graphlib,linkView)
			 	if(is_cycle_exists)
			 		return false
			 	var valid_outgoing_link = ValidateOutgoingLink(graph,linkView)
			 	if(!valid_outgoing_link)
			 		return false
		 		var valid_incoming_link = ValidateIncomingLink(graph,linkView)
		 		if(valid_incoming_link){
			    	_this.link_dialog = true
        		...
        		var link = linkView.model
        		_this.newly_created_link = link
        		_this.link_relationship =link.prop('labels/0/attrs/text/text')
        		_this.link_dialog = true
        	});
	...
	methods: {
		...
		AddNewCell(){
			performPanningActions([this.paperpanAndZoom,this.minimappanAndZoom],'reset')
			var main_div = this.$refs.main_container
			var cell = getRectangle(main_div.scrollLeft + 50,main_div.scrollTop+50,80,80,'Table')
			cell.addTo(this.graph)
		},
		ContextMenuClick(action_type){
			ContextMenuItemClick(this,action_type)
		...
		ChangeRelationships(){
			this.newly_created_link.labels([{
				    attrs: {
				        text: {
				            text: this.link_relationship
				        }
				    }
				}]);
			this.link_relationship="1:1"
